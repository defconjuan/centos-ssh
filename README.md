...
The latest CentOS-6 / CentOS-7 based releases can be pulled from the `centos-6` / `centos-7` Docker tags respectively. For production use it is recommended to select a specific release tag - the convention is `centos-6-1.7.6` OR `1.7.6` for the [1.7.6](https://github.com/defconjuan/docker-centos-ssh/tree/1.7.6) release tag and `centos-7-2.2.1` OR `2.2.1` for the [2.2.1](https://github.com/defconjuan/docker-centos-ssh/tree/2.2.1) release tag.

### Tags and respective `Dockerfile` links

- `centos-7`,`centos-7-2.2.1`,`2.2.1` [(centos-7/Dockerfile)](https://github.com/defconjuan/docker-centos-ssh/blob/centos-7/Dockerfile)
- `centos-6`,`centos-6-1.7.6`,`1.7.6` [(centos-6/Dockerfile)](https://github.com/defconjuan/docker-centos-ssh/blob/centos-6/Dockerfile)
...
For cases where access to docker exec is not possible the preferred method is to use Command Keys and the nsenter command. See [command-keys.md](https://github.com/defconjuan/docker-centos-ssh/blob/centos-6/command-keys.md) for details on how to set this up.

## Quick Example

### SSH Mode

Run up an SSH container named 'ssh.pool-1.1.1' from the docker image 'defconjuan/docker-centos-ssh' on port 2020 of your docker host.

```
$ docker run -d \
  --name ssh.pool-1.1.1 \
  -p 2020:22 \
  defconjuan/docker-centos-ssh:bt-centos-6
```
...
Run up an SFTP container named 'sftp.pool-1.1.1' from the docker image 'defconjuan/docker-centos-ssh' on port 2021 of your docker host.
...
To run the a docker container from this image you can use the standard docker commands. Alternatively, you can use the embedded (Service Container Manager Interface) [scmi](https://github.com/defconjuan/docker-centos-ssh/blob/centos-7/usr/sbin/scmi) that is included in the image since `1.7.2` / `2.1.2` or, if you have a checkout of the [source repository](https://github.com/defconjuan/docker-centos-ssh), and have make installed the Makefile provides targets to build, install, start, stop etc. where environment variables can be used to configure the container options and set custom docker run parameters.
...
$ docker run \
  ...
  --privileged \
  --volume /:/media/root \
  defconjuan/docker-centos-ssh:1.7.6 \
...
```
$ docker pull defconjuan/docker-centos-ssh:1.7.6
```
...
```
$ docker run --rm defconjuan/docker-centos-ssh \
